...
from layer.line_login.sent_user_info import SentUserInfo
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'error' in user_profile and 'expired' in user_profile['error_description']:  # noqa 501
            return line.create_error_response('Forbidden', 403)
        else:
            req_param['userId'] = user_profile['sub']
    except Exception:
        logger.exception('不正なIDトークンが使用されています')
        return line.create_error_response('Error')
    ...
    return line.create_success_response('success')
